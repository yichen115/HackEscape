...
- **键盘驱动 + 手机端原生触屏** —— 桌面视角靠鼠标交互靠键盘；手机自动展开左下虚拟摇杆 + 右下跳跃按钮 + 屏幕直接 tap 物体
...
| 操作 | 行为 |
|---|---|
| 左半屏 触摸拖动 | 虚拟摇杆 → 移动（推得越远走得越快）|
| 右半屏 拖动 | 视角 |
| 右半屏 轻点 | 与点中物体交互（替代 `E` 键 + 准星）|
| 右下"JUMP"按钮 | 跳跃 |
| 屏幕上的密码盘按钮 | 直接点；终端会弹出系统软键盘 |

设计原则：所有交互一律 `E` / 直接点击，所有数字输入一律键盘，弹窗一律 `ESC` 关闭。准星对准可交互物体时会变绿并显示标签和 `[E]` 提示（手机端隐藏准星，直接 tap 即可）。
...
└── js/
    ...
    ├── controls.js         FPS 控制器（pointer lock + WASD + 摇杆）
    ├── touch.js            手机触屏：虚拟摇杆 / 跳跃按钮 / tap 交互
    ├── collision.js        AABB 立体碰撞 + 跨步 + 落地
